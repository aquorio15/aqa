...
class FourierShift(nn.Module):
    ...
    def shift(self, x_real, x_imag):
        ...
        return torch.cat(
            [x_realUp, x_realDown, x_realLeft, x_realRight], dim=1
        ), torch.cat([x_imagUp, x_imagDown, x_imagLeft, x_imagRight], dim=1)
...
class RubikCube_multiply(nn.Module):
    ...
    def forward(self, x):
        ...
        out = torch.cat(
            [c1_processed, c2_processed, c3_processed, c4_processed, x2], dim=1
        )
...
class RefineRubik(nn.Module):
    ...
    def __init__(self, n_feat, out_channel):
        ...
        self.conv_last = nn.Sequential(
            RubikCube_multiply(n_feat, n_feat), nn.Conv2d(n_feat, out_channel, 1, 1, 0)
        )
...
class RefineShift(nn.Module):
    ...
    def __init__(self, n_feat, out_channel):
        ...
        # self.conv_last = nn.Conv2d(in_channels=n_feat, out_channels=out_channel, kernel_size=3, stride=1, padding=1)
        self.conv_last = nn.Sequential(
            FourierShift(n_feat), nn.Conv2d(n_feat, out_channel, 1, 1, 0)
        )
...
class Refine1(nn.Module):
    ...
    def __init__(self, in_channels, panchannels, n_feat):
        ...
        self.conv_last = nn.Conv2d(
            in_channels=n_feat,
            out_channels=in_channels - panchannels,
            kernel_size=3,
            stride=1,
            padding=1,
        )
